...
def main():
    ap = argparse.ArgumentParser(description="发送已渲染的桑基图视频/静态图")
    ap.add_argument("--session", required=True, choices=["midday", "close", "snapshot"])
    ...
    if not meta_path.exists():
        log.error("找不到 %s,请先 run_window/render_snapshot 渲染。", meta_path)
        sys.exit(2)
    meta = json.loads(meta_path.read_text(encoding="utf-8"))
    out = Path(meta["out"])

    subject = f"📊 {meta['date_label']} {meta['session_label']} A股板块资金流向桑基图"
